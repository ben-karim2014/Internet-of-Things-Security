...
///////////////////////////////////////////////////////////////////////////////////////
//// Dont forget to close ssl and the secket////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char *argv[])
{
	...
	for (;;)
	{
		...
		else if (retPoll > 0)
		{
			...
			{
				int count = SSL_read(ssl, buffer, MAX);
				if (count < 0)
				{
					SSL_shutdown(ssl);
					SSL_free(ssl);
...
void Send_id(){
	...
	if(SSL_write(ssl, identity_char, strlen(identity_char)) <=0){
		SSL_shutdown(ssl);
		SSL_free(ssl);
		fprintf(stderr, "Error: Failed to write ID to SSL Server: %s\n", strerror(errno));
        exit(1);
	}
}
...
void Set_ssl_connection(){
    ...
    ctx_method = SSLv23_client_method();
	
    ctx=SSL_CTX_new(ctx_method);
	if (ctx == NULL){
		fprintf(stderr, "Error: Failed creating SSL context\n");
		exit(1);
	}
    ssl = SSL_new(ctx);
	if (ssl == NULL){
		fprintf(stderr, "Error: Failed creating SSL Object\n");
		exit(1);
	}
    if (SSL_set_fd(ssl, sockfd) ==0){
		fprintf(stderr, "Error: Failed connecting SSL to Socked\n");
		exit(1);
	}
    if (SSL_connect(ssl) <=0){
		SSL_shutdown(ssl);
		SSL_free(ssl);
		fprintf(stderr, "Error: The TLS/SSL handshake was not successful\n");
		exit(1);
	}
